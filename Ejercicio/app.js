...
function parseDocument(raw){
  ...
  sections.forEach(s=>s.questions.forEach(x=>{const scale=x.text.match(/(?:escala\s*)?(?:del|de)?\s*(\d+)\s*(?:a|al|–|-)\s*(\d+)/i);if(scale){x.type='scale';x.scale={min:Number(scale[1]),max:Number(scale[2]),style:'stars'}}}));
...
function syncEditor(){
  ...
  $$('.question-edit').forEach(node=>{const q=exam.sections.flatMap(s=>s.questions).find(x=>x.id===node.dataset.id);if(!q)return;q.text=node.querySelector('.qtext').value.trim();q.type=node.querySelector('.qtype').value;if(q.type==='multiple'){q.options=[...node.querySelectorAll('.option-line')].map((r,i)=>({id:q.options[i]?.id||uid(),text:r.querySelector('.opt-text').value.trim()}));q.correct=node.querySelector('input[type=radio]:checked')?.value??''}else if(q.type==='open')q.correct=node.querySelector('.open-correct')?.value.trim()||'';else if(q.type==='scale'){let min=Number(node.querySelector('.scale-min')?.value||1),max=Number(node.querySelector('.scale-max')?.value||5);min=Math.max(0,Math.min(9,min));max=Math.max(min+1,Math.min(10,max));q.scale={min,max,style:node.querySelector('.scale-style')?.value||'stars'};q.correct=''}else q.correct=''})
}
